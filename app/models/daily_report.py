...
class WeeklyNewsletter:
    ...
    def date(self) -> str:
        ...
        return self.overview.date_end.replace("年", "-").replace("月", "-").replace("日", "")

    def to_dict(self) -> dict[str, Any]:
        return {
            "brand_name": self.brand_name,
            "overview": self.overview.to_dict(),
            "industry_news": [item.to_dict() for item in self.industry_news],
            "tech_summary": self.tech_summary.to_dict() if self.tech_summary else None,
            "generated_at": self.generated_at,
        }

    @classmethod
    def from_dict(cls, data: dict[str, Any]) -> "WeeklyNewsletter":
        tech = data.get("tech_summary")
        return cls(
            brand_name=data["brand_name"],
            overview=OverviewBlock.from_dict(data["overview"]),
            industry_news=[
                IndustryNewsItem.from_dict(i) for i in data.get("industry_news", [])
            ],
            tech_summary=TechSummarySection.from_dict(tech) if tech else None,
